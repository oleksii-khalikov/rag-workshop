...
This document presents a framework for organizing a human-agent team based on the core philosophy that human developers act as creative leads and architects, while AI agents function as force multipliers. This framework rests upon three foundational principles:
...
```mermaid
flowchart TD
    Start([Agent Scaffolder]) --> |"Writes new code, implements features,<br/>or creates boilerplate based on<br/>detailed specifications"| Test([Agent Test])
    
    Test --> |"Writes comprehensive unit tests,<br/>integration tests, and end-to-end<br/>tests for new or existing code"| Doc([Agent Documenter])
    
    Doc --> |"Generates clear, concise documentation<br/>for functions, classes, APIs,<br/>or entire codebases"| Opt([Agent Optimizer])
    
    Opt --> |"Proposes performance optimizations<br/>and code refactoring to improve<br/>readability, maintainability,<br/>and efficiency"| Sup([Agent Supervisor])
    
    Sup --> |"Provide critique to<br/>the whole process"| Dev[Human Developer<br/>Final Review & Approval]
    
    Dev --> |Merge into Mainline| Final[/Project Codebase/]
```
